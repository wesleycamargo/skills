...
Validation:
- Completed 2026-09-24: `scripts/publish-npm.sh` handles local `NPM_OTP` publication and GitHub Actions provenance publication, checks public/staged versions where the credential can inspect them, and never retries conflicts or uncertain results. The initial Actions run showed that trusted publishing cannot call `npm stage list` before the first public version, so Actions skips that unavailable preflight and relies on direct-publish conflict handling; local releases retain the staged-version guard. A trusted-publisher E401/E403/ENEEDAUTH rejection is classified without printing the npm response and directs the operator to its exact GitHub binding and direct-publish permission. `scripts/publish-npm.test.sh` covers seven mocked paths: existing version, missing OTP, local OTP, Actions provenance without staged-API access, staged version, conflict, and trusted-publisher rejection. `npm test` passed all seven publisher tests and the existing 20 tests.
